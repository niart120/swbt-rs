...
| refactor-skipped | T35: scripted no-completion waiterがDropのbounded timeout→detachをwall-clock待ちなしで検査する | worker unit | red: `cargo test --lib runtime::worker_thread::tests::drop_timeout_detaches_the_unfinished_worker_without_wall_clock_wait --locked` は `ScriptedNoCompletionWaiter` 未定義の `E0422` compile error。green: 同commandで1 test、worker-thread全9 testsが成功。dummy workerはcompletion senderを保持したままrelease barrierで停止し、scripted waiterはconnectedかつcompletionなしを確認して要求された100 msを記録し、test側の合図後に即`TimedOut`を返す。priority shutdownが`Dropped`であること、worker release前にownerのDropが完了してworkerは未終了であること、late completionが破棄済みreceiverへの`send` errorになること、release後にdetached workerが終了することをchannel順で検査した。2秒の`recv_timeout`は退行時のdeadlock watchdogにだけ使い、100 msの実時間経過を成功条件にしていない。normal completion後のjoinはT25の既存testが検査する。productionのtimeout/detach分岐とtest seamは既に分離済みのため構造変更を省略し、T35待ちだった`TimedOut`の一時`dead_code`許可だけを撤去。gate: `cargo clippy --all-targets --all-features --locked -- -D warnings`、`cargo test --all-targets --all-features --locked`（lib 214 testsとintegration/example全件）、default/no-defaultの全target各lib 214 testsとintegration/example全件、`cargo test --release --lib runtime::worker_thread::tests --locked` 9 tests、`cargo fmt --all -- --check`、`git diff --check`はすべて成功 |
| refactor-done | T36: 3件のPython `baseline_observation`とRustの意図的なsession/IMU/retry差分を同じtestで比較する | runtime characterization | red: `cargo test --lib rust_spec_delta --locked` は test-only `runtime_baseline_checkpoint` 未定義の `E0432` 3件。green: 同commandで3 tests passed。各testはcase/checkpoint IDで固定fixtureを読み、`classification = baseline_observation`を確認してから同じシナリオのRust挙動を実行する。rejected quaternion inputではPythonがrejection前にIMU epochを2秒へ進める値に対し、Rustは最後にacceptedな1秒を維持する。pre-connection Periodic inputではPython最初のcurrent reportがbutton Aを持つ値に対し、Rustはnew session開始時にinput、timer、protocol、holdoff、observationをneutral baselineへresetする。250 ms send latencyを含むbootstrap retryではPythonが初回開始から1.25秒後（完了から1秒後）に再送する値に対し、Rustは初回開始から1秒のabsolute phase（完了から750 ms）を維持し、overrunでもburstしない。refactor: fixture parse、baseline分類、case/checkpoint lookupを`runtime::test_support`の1 helperへ集約した。session/handshakeは既存deterministic testを専用名へ変更し、senderは異なる時刻での再準備を検査する既存testを残したままfixtureと同じ入力・時刻の専用testを追加した。production挙動とfixtureは変更していない。gate: Python側`cargo test --test runtime_fixture_audit --locked` 3 tests、Rust側`cargo test --release --lib rust_spec_delta --locked` 3 tests、sender 4 tests、session 2 tests、handshake 5 tests、`cargo clippy --all-targets --all-features --locked -- -D warnings`、`cargo test --all-targets --all-features --locked`（lib 215 testsとintegration/example全件）、`cargo fmt --all -- --check`、`git diff --check`はすべて成功 |
...
これは TDD item ではなく、T22 の deterministic invariant が green になった後の実時間測定で
ある。通常 test に時間閾値を入れない。

測定は clean な `8dd703fcc6bf1abc95ff6ba75510543b40c775af` に対し、Windows
`10.0.26200`、AMD64 Family 26 Model 68、32 logical CPUs、Rust
`1.96.1`、`release-test`、`no-default-features` で実行した。worker tuning は
command capacity `16`、command batch `16`、poll batch `16 × 4` である。後続
`1db0fdb` はMiri向けtest assertionだけを変更し、production pathとmeasurement harnessは
測定commitから変えていない。

```powershell
pwsh -NoProfile -File tools\measure_m2_runtime.ps1 `
  -OutputDirectory spec\wip\unit_003\evidence\activity-wait-windows-release-20260729
```

runner は開始時 clean、HEAD、branch、終了時 source status を検査し、42,002 NDJSON records
すべての JSON/schema、metric ごとの件数と連番、fairness の 16 command / 64 event workload
を検査した。raw、summary、manifest の SHA-256 はすべて一致し、manifest の
`evidence_class` は `retained` である。保存先は
`evidence/activity-wait-windows-release-20260729/` とする。
`.gitattributes` では `spec/**/evidence/**` を非 text・非 diff とし、checkout 時の
改行変換で生成時の bytes と manifest hash が変わらず、行単位の末尾空白検査が
生成物の CRLF を書式違反として扱わないようにする。

| metric | sample | p50 / p95 / p99 / max | invariant |
|---|---:|---|---|
| idle | 10.0003 s | process CPU delta `0` ticks、`0%` of one core | wait entry/return、worker wake、transport poll/event はすべて `0` |
| 8 ms jitter lateness | 10,000 ticks | `0.8475 / 1.5028 / 1.8713 / 2.2824 ms` | skip `0`、4 ms 未満の burst `0` |
| accepted interval | 9,999 intervals | `8.0112 / 9.0118 / 9.0309 / 9.3962 ms` | p99 error `+128,862 ppm` relative to 8 ms、absolute deadline phaseを維持 |
| command accept | 10,000 | `0.5 / 0.7 / 1.1 / 32.1 µs` | idle Direct workerへ1件ずつ投入 |
| command response | 10,000 | `0.9 / 1.2 / 1.7 / 32.5 µs` | enqueueからtyped response観測まで |
| transport drain | 10,000 | `0.6 / 0.7 / 1.1 / 15.9 µs` | injectごとに1 event、overflow `0` |
| idle shutdown | 1,000 | `19.2 / 44.5 / 57.1 / 107.7 µs` | priority requestからcompletion＋joinまで |
| saturated shutdown | 1,000 | `16.7 / 40.4 / 58.6 / 92.9 µs` | pending Pair＋実queue満杯、実notifier wake、completion＋join |
| fairness staging | 10,000 ticks | `285.2 / 528.3 / 834.3 / 1,465.2 µs` | deadline後、workerをgateしたまま16 command＋64 eventを配置 |
| fairness post-release accept | 10,000 ticks | `35.6 / 70.9 / 81.4 / 123.3 µs` | gate release requestからaccepted `0x30`まで |
| fairness total lateness | 10,000 ticks | p99 `894.8 µs`、max `1,532.2 µs` | 160,000 commands、640,000 events、Busy probe 10,000、error/skip/burst `0` |

fairness の reply `0x21` は 300 ms holdoff を測定へ混ぜないため意図的に reject し、
send-attempt を境界にした。post-release command completion は全 send observation を回収後に
caller が観測した保守的上限で、p99 `89.1 µs`、max `137.8 µs` である。post-release reply
attempt は p99 `69.4 µs`、max `121.3 µs` だった。

以上から M2 は activity wait と tuning `16/16/4` を採用する。10 秒 idle で測定可能な
wake/poll/CPU 増加がなく、単独command/eventと満杯shutdownの p99 は 8 ms period の 1% 未満、
fairness でも skip、burst、overflow、transport error がなかったためである。short-timeout
polling は採用しない。

測定対象は `WorkerCore`、`spawn_worker_thread` の実loop、`ChannelWorkerWaiter`、protocol、
bounded command、priority shutdown、`TestTransport` までである。`Controller` wrapper、
Bumble、USB/HCI driver、air delivery、Switch ACK、power management は測定していない。
M3 は Bumble reader から同じ activity notifier を駆動できることを entry gate で確認し、
不能なら upstream 変更または wait design の再判断を行う。M5 は実adapter/Switchのlatency、
M8 はlong-run jitterとpower managementを再測定する。
...
| path | change | 内容 |
|---|---|---|
| `.gitattributes` | modify | retained evidence を改行変換せず生成時の bytes のまま保持し、生成物を行単位 diff から除外 |
| `Cargo.toml` | modify | profile runtimeでも使う`serde_json`をdev-onlyから通常依存へ移動。Bumble feature境界は維持 |
| `src/adapter.rs` / `src/connection.rs` / `src/diagnostics.rs` | modify | adapter selector、connection result、public lifecycle/status projection |
| `src/controller/` | modify / new | public builder/controller、config、client、reporting 固有 method、release measurement harness |
...
| `tools/measure_m2_runtime.ps1` | new | clean source gate、release test起動、外部CPU採取、NDJSON検証、hash manifest |
| `spec/complete/unit_003/evidence/activity-wait-windows-release-20260729/` | new | retained raw NDJSON、summary、manifest、manifest hash |
| `README.md` | modify | fake-verified M2 surfaceと未実装 transport の明記 |
| `spec/initial/api.md` | modify | explicit close/Dropとnew-session neutral resetの公開契約 |
| `spec/dev-journal.md` | modify | wait decision、M3/M6へ残す境界、未検証事項 |
| `spec/complete/unit_003/M2_RUNTIME.md` | new / modify | 本作業仕様と検証記録 |
...
|---|---|---|
| `cargo fmt --all -- --check` | passed | rustfmt差分なし |
...
| `cargo test --doc --all-features --locked` | passed | doctest 1 |
| `cargo build --locked` | passed | default production build |
| `cargo build --all-features --locked` | passed | Bumble feature production build |
| `cargo +1.87 build --no-default-features --locked` | passed | MSRV Bumble-free production build |
| `cargo tree --no-default-features --edges normal --locked` | passed | `serde_json` treeだけでBumble不在 |
| `cargo +nightly miri test --lib --no-default-features --locked protocol::` | passed | pure protocol 63 tests |
| `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps --all-features --locked` | passed | rustdoc warning 0 |
| full activity wait measurement command | passed | retained 42,002 records、source/hash/schema/count検査成功 |
...
1件失敗した。wire契約のexact assertionを維持し、orientationだけ
`8 × f64::EPSILON`以内で比較するtestへ狭めた後、targeted testとprotocol 63件を再実行して
成功した。production codeとwire bytesは変更していない。
...
- [x] 6 model × reporting の fake-runtime harness を通した
- [x] acceptance 前後の commit と ACK ordering を検証した
- [x] handshake / Ready を fake event の因果順で検証した
- [x] lifecycle、stale session、backpressure、panic、join を検証した
- [x] Drop の bounded shutdown と正常 fake の thread 回収を検証した
- [x] builder/profile orchestration と失敗 cleanup を検証した
- [x] Python runtime fixture と `rust_spec_delta` を区別して記録した
- [x] activity wait の測定値と選択理由を記録した
