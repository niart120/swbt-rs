...
- state-changing reply は `PreparedSessionReply { reply, next_session }` を返す。内包する
  `PreparedSubcommandReply` が bytes と next timer を保持し、M2 が送信受理後に両方を
  commit する。M1 は transport 受理を模擬しない。
- facade は parse 不能を外側の `ProtocolError`、parse 済み subcommand の意味エラーを
  `OutputPreparation::Subcommand.outcome` に分ける。後者では packet、raw rumble、borrowed
  request を保持し、M2 が Python と同じ観測順序を再 parse なしで実装できるようにする。
- facade が所有するのは immutable な device info address と `VirtualSpiFlash<M>` だけとする。
  session、timer、入力状態の選択、送信受理後 commit は M2 に置く。
...
| `cargo +1.87 test --locked --lib protocol::tests::imu` | pass | red は calibration 引数、quaternion mode、state getter 不在。green は全4 mode、identity、正負、3 sample と順序、較正、時計逆行、reset、bit/timestamp 境界を含む 8 passed |
| `cargo test --release --locked --lib protocol::tests::imu` | pass | TDD item 9 の 8 tests。最適化後も Python fixture bytes と一致 |
...
| `cargo +1.87 test --locked --lib protocol::tests::stateful_subcommand` | pass | red は stateful handler / reply 型 / error variant 不在。green は exact fixture、candidate session、current 不変、全 model × IMU mode、readiness、同一 mode reset、引数検査、非中立 prefix、timer、Python error 表示を含む 11 passed |
