...
- concrete attempt の cleanup 自身が失敗した場合は、primary と cleanup の source を利用者が
  辿れる error 境界を T33 で決める。非公開 field に保持するだけで完了扱いにしない。
...
| planned | T33: 6 model×reportingの共通smokeがbuild→fake Ready→最小input→closeを通り、concrete attemptのcleanup error mappingを固定する | integration | |
