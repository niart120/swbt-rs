...
//! pairing with a physical adapter and Switch remains unverified. With the
//! `bumble` feature, profile creation publishes a valid empty envelope without
//! replacing an existing target before it opens the adapter and waits for
//! pairing readiness. Feature-disabled profile creation stops before creating
//! a file. Pairing-key persistence and reconnect from an existing profile
//! remain unavailable. Explicit close waits for cleanup completion, joins the
//! worker, and returns cleanup or join failures.
